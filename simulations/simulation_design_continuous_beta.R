...
designs = rbind(
  c("as.numeric(ws==1)*as.numeric(.45-ws+.5*us+.5*rnorm(n)+xs>=0)", "rbeta(n,2,5)","as.numeric(runif(n)>0.5)", .7, .3,"rexp(n,.33)", "function(t){t}"),#40%cens
  c("as.numeric(ws==1)*as.numeric(.45-ws+.5*us+.5*rnorm(n)+xs>=0)", "rbeta(n,2,5)","as.numeric(runif(n)>0.5)", .7, .3,"rexp(n,.87)", "function(t){t}")#20%cens
)
